...
export const server: Plugin = async ({ directory, client, $, worktree }) => {
  ...
  const port = await startServer(projectDirectory, DEFAULT_PORT)
  ...
  await client.app.log({
    body: {
      ...
      level: "info",
      message: `Preview server started at ${baseUrl}`,
      extra: { directory: projectDirectory, port },
    },
  })
  ...
  return {
    tool: {
      preview: tool({
        ...
        args: {
          ...
          dir: tool.schema.string().optional().describe("Absolute path to a project directory to preview from (overrides default)"),
    ...
    event: async ({ event }) => {
      ...
      if (!filePath.endsWith(".md") && !filePath.endsWith(".drawio")) {
        return
      }
